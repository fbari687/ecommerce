            ...
            <form action="/dashboard/librarians/{{ $admin->f_id }}" method="POST"
                ...
                <div class="flex flex-col gap-2 text-white">
                    ...
                    <input type="text" name="f_nama" id="f_nama" class="bg-gray-950 py-2 px-3 rounded-md"
                        placeholder="Nama" value="{{ old('f_nama', $admin->f_nama) }}">
                    @error('f_nama')
                ...
                <div class="flex flex-col gap-2 text-white">
                    ...
                    <input type="text" name="f_username" id="f_username" class="bg-gray-950 py-2 px-3 rounded-md"
                        placeholder="Username" value="{{ old('f_username', $admin->f_username) }}">
                    @error('f_username')
                        <p class="text-red-500 px-3 text-xs">{{ $message }}</p>
                    @enderror
                </div>
                <div class="flex flex-col gap-2 text-white">
                    ...
                    <select name="f_level" id="f_level" class="bg-gray-950 py-2 px-3 rounded-md">
                        ...
                        <option value="Admin" {{ 'Admin' == old('f_level', $admin->f_level) ? 'selected' : '' }}>Admin
                        </option>
                        <option value="Pustakawan" {{ 'Pustakawan' == old('f_level', $admin->f_level) ? 'selected' : '' }}>
                            Pustakawan</option>
                    </select>
                    @error('f_level')
                        <p class="text-red-500 px-3 text-xs">{{ $message }}</p>
                ...
                <div class="flex flex-col gap-2 text-white">
                    ...
                    <select name="f_status" id="f_status" class="bg-gray-950 py-2 px-3 rounded-md">
                        ...
                        <option value="Aktif" {{ 'Aktif' == old('f_status', $admin->f_status) ? 'selected' : '' }}>Aktif
                        </option>
                        <option value="Tidak Aktif"
                            {{ 'Tidak Aktif' == old('f_status', $admin->f_status) ? 'selected' : '' }}>
                            Tidak Aktif</option>
                    </select>
                    @error('f_status')
                        <p class="text-red-500 px-3 text-xs">{{ $message }}</p>
